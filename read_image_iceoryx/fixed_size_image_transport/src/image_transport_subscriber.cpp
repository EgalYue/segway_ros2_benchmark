...
#include "fixed_size_msgs/msg/image1m.hpp"
...
int main(int argc, char ** argv)
{
  ...
  if (image_id == "1m") {
    ImageSubscriber <fixed_size_msgs::msg::Image1m> sub(node, "image1m", show_gui);
    sub.run();
  } else if (image_id == "VGA") {
    ImageSubscriber<fixed_size_msgs::msg::ImageVGA> sub(node, "imageVGA", show_gui);
    sub.run();
  } else if (image_id == "720p") {
    ImageSubscriber<fixed_size_msgs::msg::Image720p> sub(node, "image720p", show_gui);
    sub.run();
  } else if (image_id == "1080p") {
    ImageSubscriber<fixed_size_msgs::msg::Image1080p> sub(node, "image1080p", show_gui);
    sub.run();
  } else if (image_id == "4k") {
    ImageSubscriber<fixed_size_msgs::msg::Image4k> sub(node, "image4k", show_gui);
    sub.run();
  } else {
    RCLCPP_ERROR(
      node->get_logger(),
