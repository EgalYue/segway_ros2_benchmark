...
int main(int argc, char ** argv)
{
  ...
  if (image_id == "1m") {
      ImagePublisher<fixed_size_msgs::msg::Image1m> pub(node, "image1m");
      pub.run(frequency);
  } else if (image_id == "VGA") {
    ImagePublisher<fixed_size_msgs::msg::ImageVGA> pub(node, "imageVGA");
    pub.run(frequency);
  } else if (image_id == "720p") {
    ImagePublisher<fixed_size_msgs::msg::Image720p> pub(node, "image720p");
    pub.run(frequency);
  } else if (image_id == "1080p") {
    ImagePublisher<fixed_size_msgs::msg::Image1080p> pub(node, "image1080p");
    pub.run(frequency);
  } else if (image_id == "4k") {
    ImagePublisher<fixed_size_msgs::msg::Image4k> pub(node, "image4k");
    pub.run(frequency);
  } else {
    RCLCPP_ERROR(
      node->get_logger(),
